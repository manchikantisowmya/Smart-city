(function () {
    ...
    $(document).ready(function () {
        ...
        async function fetchConfig() {
            try {
                const response = await fetch('config.json'); // Adjust the path as necessary
                const config = await response.json();
                baseUrl = config.development.BASE_URL;
            } catch (error) {
        ...
        fetchConfig().then(() => {
            fetchMissionData().then(() => {
                ...
                if (missionJSON && missionJSON != "\"\"" && missionJSON != "{}") {
                    ...
                    cesiumMissionManager.loadJsonMission(missionJSON, missionJSON.mission_id);
                } else {
