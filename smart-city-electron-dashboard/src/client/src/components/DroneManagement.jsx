...
import { Box, Button, Table, Grid, TableBody, TableCell, TableHead, TableRow, Modal, Typography, TextField, Snackbar, Alert, Tabs, Tab, AppBar } from '@mui/material';
...
export default function DroneManagement() {
  ...
  const handleAddDroneSubmit = async () => {
    try {
      await addDrone(newDroneData);
      setDrones([...drones, newDroneData]);
      setSnackbarMessage(`Drone added successfully! Drone ID: ${newDroneData.drone_id}`);
      setIsAddDroneModalOpen(false);  // Close the modals
      setShowSuccessSnackbar(true); // Show the success snackbar
