class MissionManager {
    ...
    simulateMission() {
        ...
        Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIzMWJkNmFiMi1lZmYyLTQ5ZmUtYTMwNC05Njk4ZDZmMGJhMmMiLCJpZCI6MTQ4Nzk3LCJpYXQiOjE2ODc1NzgyMzJ9.11e3lktXLvEOJTJSQmW5V4moqStiyt9xPUcYjXpMZk0';
        ...
        */
        ...
        const start = Cesium.JulianDate.fromIso8601("2023-10-28T23:10:00Z");
        ...
        async function loadModel() {
            ...
            const airplaneUri = await Cesium.IonResource.fromAssetId(2081359);
            const airplaneEntity = viewer.entities.add({
                availability: new Cesium.TimeIntervalCollection([ new Cesium.TimeInterval({ start: start, stop: stop }) ]),
                position: positionProperty,
                // Attach the 3D model instead of the green point.
                model: { uri: airplaneUri, scale: 2 },
                // Automatically compute the orientation from the position.
                orientation: new Cesium.VelocityOrientationProperty(positionProperty),    
                path: new Cesium.PathGraphics({ width: 3, material: Cesium.Color.CRIMSON} )
            });
    ...
    resetSimulation(){
        ...
        viewer.clock.currentTime = viewer.clock.startTime.clone(); // Reset current time to start
        viewer.clock.shouldAnimate = true;
    }
    ...
    getJsonMission() {
        ...
        const mission = {
            ...
            mission_distance: this.domManager.missionManager.sumDistance,
            mission_name: this.domManager.name,
            mission_description : this.mission_description,
            mission_start_time: this.mission_start_time,
            mission_end_time: this.mission_end_time
        }
    ...
    loadJsonMission(jsonMission, missionName) {
        ...
        if (!loadedMission.mission_waypoints) {
            missionItems = this.prepareV1MissionItems(loadedMission)
        } else if (loadedMission.mission_waypoints) {
            this.loadMissionDefaults(loadedMission.mission_global_settings)
            loadedMission.items= loadedMission.mission_waypoints.map((waypoint, index) => ({
